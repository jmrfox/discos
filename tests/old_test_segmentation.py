...
class TestMeshSegmenterVolumeConservation:
    ...
    def test_volume_conservation_full_length(
        self, test_cylinder, test_cylinder_params, theoretical_volume
    ):
        ...
        volume_error_percent = (
            abs(total_segment_volume - theoretical_volume) / theoretical_volume * 100
        )

        # Volume should be conserved within 5%
        assert (
            volume_error_percent < 5.0
        ), f"Volume error {volume_error_percent:.2f}% exceeds 5%"

        # Check that segment volume is close to theoretical
        assert (
            abs(graph.segments_list[0].volume - theoretical_volume) / theoretical_volume
            < 0.05
        )

    def test_volume_conservation_half_length(
        self, test_cylinder, test_cylinder_params, theoretical_volume
    ):
        ...
        for segment in graph.segments_list:
            segment_error = (
                abs(segment.volume - expected_segment_volume) / expected_segment_volume
            )
            assert (
                segment_error < 0.1
            ), f"Individual segment volume error {segment_error:.2f} exceeds 10%"

    def test_volume_conservation_third_length(
        self, test_cylinder, test_cylinder_params, theoretical_volume
    ):
...
class TestMeshSegmenterSurfaceAreaConservation:
    ...
    def test_surface_area_conservation_full_length(
        self, test_cylinder, test_cylinder_params, theoretical_surface_area
    ):
        ...
        area_error_percent = (
            abs(total_external_area - theoretical_surface_area)
        ...
        ), f"Surface area error {area_error_percent:.2f}% exceeds 5%"

    def test_surface_area_conservation_half_length(
        self, test_cylinder, test_cylinder_params, theoretical_surface_area
    ):
        ...
        ), "Internal surface area should be created from cuts"

    def test_surface_area_conservation_third_length(
        self, test_cylinder, test_cylinder_params, theoretical_surface_area
    ):
...
class TestMeshSegmenterIntegration:
    ...
    def test_comprehensive_cylinder_segmentation(self):
        ...
        for slice_height, expected_count in zip(slice_heights, expected_segments):
            ...
            ), f"Expected {expected_count} segments, got {len(segments)} for slice_height={slice_height}"

            # Check volume conservation
            total_volume = sum(seg.volume for seg in segments)
            volume_error = (
                abs(total_volume - theoretical_volume) / theoretical_volume * 100
            )
            assert (
                volume_error < 5.0
            ), f"Volume error {volume_error:.2f}% for slice_height={slice_height}"
            ...
            assert (
                area_error < 5.0
            ), f"Surface area error {area_error:.2f}% for slice_height={slice_height}"

            # Check that all segments have positive volumes and surface areas
            for i, segment in enumerate(segments):
                assert (
                    segment.volume > 0
                ), f"Segment {i} has non-positive volume: {segment.volume}"
                assert (
                    segment.external_surface_area > 0
                ), f"Segment {i} has non-positive external surface area"

                # Check segment bounds
                assert (
                    segment.z_min < segment.z_max
                ), f"Segment {i} has invalid z-bounds: {segment.z_min} >= {segment.z_max}"
    ...
    def test_segment_graph_creation(self):
        ...
        # Get segment graph
        segment_graph = segmenter.segment_mesh(
            cylinder, slice_height=5.0, return_segment_graph=True
        )
    ...
    def test_different_cylinder_sizes(self):
        ...
        for case in test_cases:
            cylinder = create_cylinder_mesh(
                length=case["length"], radius=case["radius"], resolution=24
            )
            ...
            assert (
                volume_error < 5.0
            ), f"Volume error {volume_error:.2f}% for case {case}"
...
class TestMeshSegmenterErrorHandling:
    ...
    def test_very_small_slice_height(self):
        ...
        assert (
            volume_error < 10.0
        )  # Allow slightly higher tolerance for many small segments
...
class TestMeshSegmenterTorusConservation:
    ...
    def test_torus_volume_conservation_half_extent(
        self, test_torus, test_torus_params, theoretical_volume, torus_extent
    ):
        ...
        assert (
            volume_error_percent < 10.0
        ), f"Torus volume error {volume_error_percent:.2f}% exceeds 10%"

        # Check that all segments have positive volume
        for i, segment in enumerate(segments):
            assert (
                segment.volume > 0
            ), f"Torus segment {i} has non-positive volume: {segment.volume}"

    def test_torus_volume_conservation_third_extent(
        self, test_torus, test_torus_params, theoretical_volume, torus_extent
    ):
    ...
    def test_torus_volume_conservation_quarter_extent(
        self, test_torus, test_torus_params, theoretical_volume, torus_extent
    ):
    ...
    def test_torus_surface_area_conservation_half_extent(
        self, test_torus, theoretical_surface_area, torus_extent
    ):
        ...
        assert (
            area_error_percent < 10.0
        ), f"Torus surface area error {area_error_percent:.2f}% exceeds 10%"
    ...
    def test_torus_surface_area_conservation_third_extent(
        self, test_torus, theoretical_surface_area, torus_extent
    ):
    ...
    def test_torus_surface_area_conservation_quarter_extent(
        self, test_torus, theoretical_surface_area, torus_extent
    ):
    ...
    def test_torus_comprehensive_segmentation(
        self,
        test_torus,
        test_torus_params,
        theoretical_volume,
        ...
        for slice_height in slice_heights:
            ...
            assert (
                len(segments) > 0
            ), f"No segments created for torus with slice_height={slice_height}"
            ...
            assert (
                volume_error < 10.0
            ), f"Torus volume error {volume_error:.2f}% for slice_height={slice_height}"
            ...
            assert (
                area_error < 10.0
            ), f"Torus surface area error {area_error:.2f}% for slice_height={slice_height}"

            # Check segment properties
            for i, segment in enumerate(segments):
                assert (
                    segment.volume > 0
                ), f"Torus segment {i} has non-positive volume for slice_height={slice_height}"
                assert (
                    segment.external_surface_area > 0
                ), f"Torus segment {i} has non-positive external surface area"
                assert (
                    segment.z_min < segment.z_max
                ), f"Torus segment {i} has invalid z-bounds"
    ...
    def test_torus_topology_challenges(self, test_torus, torus_extent):
        ...
        for i, segment in enumerate(segments):
            ...
            assert (
                segment.mesh.is_watertight or not segment.mesh.is_watertight
            ), "Segment mesh validity check"

            # Check centroid is reasonable
            assert not np.any(
                np.isnan(segment.centroid)
            ), f"Segment {i} has NaN centroid"
            assert not np.any(
                np.isinf(segment.centroid)
            ), f"Segment {i} has infinite centroid"
