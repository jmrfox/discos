...
class TestSWCData:
    ...
    def test_swc_data_file_writing(self, sample_swc_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                "# SWC file generated from SegmentGraph for Arbor simulator" in content
            )
...
class TestSegmentGraphSWCExport:
    ...
    def simple_segment_graph(self):
        ...
        graph.add_node(
            "seg_0_0", centroid=np.array([0.0, 0.0, 0.0]), volume=10.0, slice_index=0
        )
        graph.add_node(
            "seg_0_1", centroid=np.array([1.0, 0.0, 0.5]), volume=8.0, slice_index=0
        )
        graph.add_node(
            "seg_1_0", centroid=np.array([2.0, 0.0, 1.0]), volume=6.0, slice_index=1
        )
        graph.add_node(
            "seg_1_1", centroid=np.array([3.0, 0.0, 1.5]), volume=4.0, slice_index=1
        )
